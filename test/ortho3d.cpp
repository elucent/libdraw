...
int main(int argc, char** argv) {
    ...
    cube(-64, -8, -64, 128, 16, 128, actex(block));
    ...
    while (running()) {
        ...
        // set camera
        ortho(width(SCREEN) / 2, height(SCREEN) / 2);
        look(0, 0, 0, yaw, pitch);
        pan(width(SCREEN) / 4, height(SCREEN) / 4, 0);

        // draw scene
        render(world, block);

        color(RED);
        cube(0, 8, 0, 16, 4, 4, sctex(BLANK));
        cube(8, 8, 0, 4, 6, 6, sctex(BLANK));
        color(BLUE);
        cube(0, 8, 0, 4, 4, 16, sctex(BLANK));
        cube(0, 8, 8, 6, 6, 4, sctex(BLANK));
        color(WHITE);
